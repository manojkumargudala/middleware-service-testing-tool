...
public class BasicResponseRenderer implements ResponseRenderer {
  ...
  public Response render(final ResponseDefinition responseDefinition) {
    ...
    HttpHeaders httpHeaders =
        Optional.fromNullable(responseDefinition.getHeaders()).or(new HttpHeaders())
            .plus(corsHeader);

    return response().status(responseDefinition.getStatus()).headers(httpHeaders)
        .body(responseDefinition.getBody()).build();
  }
}
