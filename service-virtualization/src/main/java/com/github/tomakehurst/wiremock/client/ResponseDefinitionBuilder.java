...
public class ResponseDefinitionBuilder {
  ...
  public static ResponseDefinitionBuilder like(final ResponseDefinition responseDefinition) {
    ResponseDefinitionBuilder builder = new ResponseDefinitionBuilder();
    builder.status = responseDefinition.getStatus();
    builder.headers =
        responseDefinition.getHeaders() != null ? newArrayList(responseDefinition.getHeaders()
            .all()) : Lists.<HttpHeader>newArrayList();
  ...
  public static ResponseDefinition jsonResponse(final Object body) {
    return new ResponseDefinitionBuilder().withBody(Json.write(body)).withStatus(HTTP_OK)
  ...
  public ResponseDefinitionBuilder withStatus(final int status) {
    this.status = status;
    return this;
  }

  public ResponseDefinitionBuilder withHeader(final String key, final String value) {
    headers.add(new HttpHeader(key, value));
    return this;
  }

  public ResponseDefinitionBuilder withBodyFile(final String fileName) {
    this.bodyFileName = fileName;
    return this;
  }

  public ResponseDefinitionBuilder withBody(final String body) {
    this.bodyContent = body.getBytes(Charset.forName(UTF_8.name()));
    isBinaryBody = false;
    return this;
  }

  public ResponseDefinitionBuilder withBody(final byte[] body) {
    this.bodyContent = body;
    isBinaryBody = true;
    return this;
  }

  public ResponseDefinitionBuilder withFixedDelay(final Integer milliseconds) {
    this.fixedDelayMilliseconds = milliseconds;
    return this;
  }

  public ResponseDefinitionBuilder withTransformers(final String... responseTransformerNames) {
    this.responseTransformerNames = asList(responseTransformerNames);
    return this;
  }

  public ProxyResponseDefinitionBuilder proxiedFrom(final String proxyBaseUrl) {
  ...
  public static class ProxyResponseDefinitionBuilder extends ResponseDefinitionBuilder {
    ...
    private final List<HttpHeader> additionalRequestHeaders = newArrayList();

    public ProxyResponseDefinitionBuilder(final ResponseDefinitionBuilder from) {
    ...
    public ProxyResponseDefinitionBuilder withAdditionalRequestHeader(final String key,
        final String value) {
  ...
  public ResponseDefinitionBuilder withFault(final Fault fault) {
    this.fault = fault;
    return this;
  }
  ...
  public ResponseDefinition build() {
    ...
    if (isBinaryBody) {
      ...
      if (bodyContent == null) {
        ...
        response =
            new ResponseDefinition(status, new String(bodyContent, Charset.forName(UTF_8.name())));
      }
    }
