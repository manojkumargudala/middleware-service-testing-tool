...
public class WireMockServerRunner {
  ...
  static {
    System.setProperty("org.mortbay.log.class",
        "com.github.tomakehurst.wiremock.jetty.LoggerAdapter");
  }

  private WireMockServer wireMockServer;
  private UserDefinedProjectDetailsDto usrDfndDtsDto;
  ...
  public void runInBackground(final VirtualizationUiOptions options,
      final UserDefinedProjectDetailsDto usrDto) {
    usrDfndDtsDto = usrDto;
    FileSource fileSource = options.filesRoot();
    fileSource.createIfNecessary();
    FileSource filesFileSource = fileSource.child(FILES_ROOT);
    filesFileSource.createIfNecessary();
    ...
    if (options.recordMappingsEnabled()) {
      if (options.isAddToProject()) {
        wireMockServer.enableRecordMappings(mappingsFileSource, filesFileSource, usrDto, options);
      } else {
        wireMockServer.enableRecordMappings(mappingsFileSource, filesFileSource);
      }
    }
  ...
  private void addProxyMapping(final String baseUrl) {
    wireMockServer.loadMappingsUsing(new MappingsLoader() {
      public void loadMappingsInto(final StubMappings stubMappings) {
        ...
        proxyBasedMapping.setPriority(10); // Make it low priority so
        // that existing stubs will
        // take precedence
