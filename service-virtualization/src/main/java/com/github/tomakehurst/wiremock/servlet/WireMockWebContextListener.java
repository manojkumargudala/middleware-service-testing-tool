...
public class WireMockWebContextListener implements ServletContextListener {
  ...
  public void contextInitialized(final ServletContextEvent sce) {
    ...
    boolean verboseLoggingEnabled =
        Boolean.parseBoolean(fromNullable(context.getInitParameter("verboseLoggingEnabled")).or(
            "true"));

    JsonFileMappingsLoader defaultMappingsLoader =
        new JsonFileMappingsLoader(fileSource.child("mappings"));
    MappingsSaver mappingsSaver = new NotImplementedMappingsSaver();
    WireMockApp wireMockApp =
        new WireMockApp(new NotImplementedRequestDelayControl(), false, defaultMappingsLoader,
            mappingsSaver, false, maxRequestJournalEntries,
            Collections.<String, ResponseTransformer>emptyMap(), fileSource,
   ...
   */
  private Optional<Integer> readMaxRequestJournalEntries(final ServletContext context) {
