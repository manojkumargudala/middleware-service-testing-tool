...
public class HttpAdminClient implements Admin {
  ...
  public void addStubMapping(final StubMapping stubMapping) {
    postJsonAssertOkAndReturnBody(urlFor(NewStubMappingTask.class), Json.write(stubMapping),
        HTTP_CREATED);
  }
  ...
  public VerificationResult countRequestsMatching(final RequestPattern requestPattern) {
    String body =
        postJsonAssertOkAndReturnBody(urlFor(GetRequestCountTask.class),
            Json.write(requestPattern), HTTP_OK);
  ...
  @Override
  public void addSocketAcceptDelay(final RequestDelaySpec spec) {
  ...
  private String postJsonAssertOkAndReturnBody(final String url, final String json,
    ...
    try {
      ...
      if (statusCode != expectedStatus) {
        throw new VerificationException("Expected status " + expectedStatus + " for " + url
            + " but was " + statusCode);
      }
  ...
  private String getJsonAssertOkAndReturnBody(final String url, final int expectedStatus) {
