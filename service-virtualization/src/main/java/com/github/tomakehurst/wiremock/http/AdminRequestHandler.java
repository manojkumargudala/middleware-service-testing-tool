...
public class AdminRequestHandler extends AbstractRequestHandler {
  ...
  public AdminRequestHandler(final Admin admin, final ResponseRenderer responseRenderer) {
  ...
  public ResponseDefinition handleRequest(final Request request) {
    ...
    AdminTask adminTask =
        AdminTasks.taskFor(request.getMethod(), withoutAdminRoot(request.getUrl()));
    return adminTask.execute(admin, request);
  }

  private static String withoutAdminRoot(final String url) {
