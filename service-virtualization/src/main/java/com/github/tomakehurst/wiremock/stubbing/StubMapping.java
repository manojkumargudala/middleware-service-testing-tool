...
public class StubMapping {
  ...
  public StubMapping(final RequestPattern requestPattern, final ResponseDefinition response) {
  ...
  public static StubMapping buildFrom(final String mappingSpecJson) {
    return Json.read(mappingSpecJson, StubMapping.class);
  }

  public static String buildJsonStringFor(final StubMapping mapping) {
  ...
  public int comparePriorityWith(final StubMapping otherMapping) {
  ...
  private void assertIsInValidState() {
    if (serviceType != null) {
      if (serviceType.equalsIgnoreCase("Soap")) {
        if (!(request.getMethod().compareTo(RequestMethod.POST) == 0)) {
          throw new IllegalStateException(
              "if service type is soap then request method type should be POST");
        }
      }
    }
  }
