...
public class WireMockApp implements StubServer, Admin {
  ...
  public WireMockApp(final RequestDelayControl requestDelayControl,
      final boolean browserProxyingEnabled, final MappingsLoader defaultMappingsLoader,
      final MappingsSaver mappingsSaver, final boolean requestJournalDisabled,
      final Optional<Integer> maxRequestJournalEntries,
      final Map<String, ResponseTransformer> transformers, final FileSource rootFileSource,
    ...
    requestJournal =
        requestJournalDisabled ? new DisabledRequestJournal() : new InMemoryRequestJournal(
  ...
  @Override
  public ResponseDefinition serveStubFor(final Request request) {
    ResponseDefinition baseResponseDefinition = stubMappings.serveFor(request);
    requestJournal.requestReceived(request);

    ResponseDefinition responseDefinition =
        applyTransformations(request, baseResponseDefinition,
            ImmutableList.copyOf(transformers.values()));

    if (!responseDefinition.wasConfigured() && request.isBrowserProxyRequest()
  ...
  private ResponseDefinition applyTransformations(final Request request,
      final ResponseDefinition responseDefinition, final List<ResponseTransformer> transformers) {
    ...
    ResponseDefinition newResponseDef =
        transformer.applyGlobally() || responseDefinition.hasTransformer(transformer) ? transformer
            .transform(request, responseDefinition, rootFileSource.child(FILES_ROOT))
            : responseDefinition;
